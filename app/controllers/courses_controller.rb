class CoursesController < ApplicationController
  ...
  def create
    ...
    if @course.save
      ...
      redirect_to courses_path, flash: {success: "新主机创建成功"}
    else
  ...
  def destroy
    ...
    @course.destroy
    flash={:success => "成功删除主机: #{@course.name}"}
    redirect_to courses_path, flash: flash
  end
  ...
  def open
    ...
    redirect_to courses_path, flash: {success: "已经成功开通该主机:#{ @course.name}"}
  end
  ...
  def close
    ...
    redirect_to courses_path, flash: {success: "已经成功关闭该主机:#{ @course.name}"}
  end
  def selected
    @course = Course.find_by_id(params[:id])
  ...
  def select
    ...
    flash={:success => "成功选择主机: #{@course.name}"}
    redirect_to courses_path, flash: flash
  end
  ...
  def open_visit
    ...
    flash={:success => "成功申请访客: #{@course.name}"}
    redirect_to courses_path, flash: flash
  end
  ...
  def swap
    ...
    @course=Course.find_by_id(params[:id])
    current_user.courses<<@course
    flash={:success => "成功预约: #{@course.name} 成功取消: #{@info}"}
    redirect_to courses_path, flash: flash
  end
  ...
  def quit
    ...
    flash={:success => "成功取消: #{@course.name}"}
    redirect_to courses_path, flash: flash
  end
  def show
    @course=Course.find_by_id(params[:id])
